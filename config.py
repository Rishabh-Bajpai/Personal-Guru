...
def _determine_session_uid():
    """Return a stable uid suffix when available."""
...
class Config:
    ...
    _session_uid = _determine_session_uid()
    SESSION_FILE_DIR = os.environ.get("SESSION_FILE_DIR") or os.path.join(
        tempfile.gettempdir(), f"personal-guru-flask-session-{_session_uid}"
    )
    ...
    ENABLE_TELEMETRY = os.environ.get("ENABLE_TELEMETRY", "False").lower() == "true"
    ENABLE_TELEMETRY_LOGGING = (
        os.environ.get("ENABLE_TELEMETRY_LOGGING", "True").lower() == "true"
    )
    SANDBOX_PATH = os.environ.get("SANDBOX_PATH") or os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "data", "sandbox"
    )
