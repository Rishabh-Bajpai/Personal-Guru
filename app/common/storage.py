...
def save_topic(topic_name, data):
    ...
    try:
        ...
        if reorder_needed:
            logging.info(
                f"Topic {topic_name}: Reordering detected. Shifting indices to temporary safe space."
            )
        ...
        for step_data in incoming_msg_data:
            ...
            if step:
                ...
                if new_title and new_title != step.title:
                    ...
                    if not step_data.get("content") and not step_data.get(
                        "teaching_material"
                    ):
                        step.content = None
                        step.questions = None
                ...
                new_content = step_data.get("teaching_material") or step_data.get(
                    "content"
                )
                if new_content:
                    step.content = new_content
                    logger.info(
                        f"DEBUG save_topic: Step {step_index} - saved content, length: {len(step.content) if step.content else 0}"
                    )
                else:
                    logger.info(
                        f"DEBUG save_topic: Step {step_index} - no content to save. Keys: {list(step_data.keys())}"
                    )
...
def save_chat_history(
    ...
    try:
        ...
        if not topic.chat_mode:
            chat_session = ChatMode(
                user_id=current_user.userid, topic_id=topic.id, history=[]
            )
            db.session.add(chat_session)
        else:
            chat_session = topic.chat_mode
...
def load_topic(topic_name, update_timestamp=False):
    ...
    data = {
        ...
        "chat_history_summary": (topic.chat_mode.history_summary or [])
        if topic.chat_mode
        else [],
        "popup_chat_history": (topic.chat_mode.popup_chat_history or [])
        if topic.chat_mode
        else [],
        "chapter_mode": [],
        "quiz_mode": None,
    ...
    for i in range(len(plan)):
        ...
        if step_model:
            ...
            if step_model.podcast_audio_path:
                ...
                logging.info(
                    f"DEBUG: Processing audio for step {step_model.step_index}. Raw path: {audio_path}"
                )
                ...
                if os.path.exists(audio_path):
                    try:
                        with open(audio_path, "rb") as audio_file:
                            ...
                            logging.info(
                                f"DEBUG: Successfully loaded and encoded audio for step {step_model.step_index}"
                            )
                    except Exception as e:
                        logging.warning(
                            f"Failed to load audio file for topic {topic_name} step {step_model.step_index}: {e}"
                        )
                else:
                    logging.warning(
                        f"Audio file not found at path: {step_model.podcast_audio_path} or resolved path {audio_path}"
                    )
    ...
    if topic.quiz_mode:
        ...
        data["quiz_mode"] = {
            ...
            "date": latest_quiz.created_at.isoformat()
            if latest_quiz.created_at
            else None,
...
def get_all_topics():
    ...
    try:
        ...
        topics = (
            ...
            .order_by(Topic.modified_at.desc())
            .all()
        )
...
def get_topics_metadata(limit=None):
    """
    ...
    try:
        query = Topic.query.filter_by(user_id=current_user.userid).order_by(
            Topic.modified_at.desc()
        )
