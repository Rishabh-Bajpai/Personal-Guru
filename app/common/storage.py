...
def save_topic(topic_name, data):
    ...
    try:
        ...
        # Note: ChapterMode relation is 'cascade="all, delete-orphan"', so we need
        # to be careful.
        ...
        for step_data in incoming_msg_data:
            ...
            if (
                ...
                and step_index in existing_steps_by_index
            ):
            ...
            if step:
                ...
                step.content = (
                    ...
                    or step_data.get("teaching_material")
                    or step.content
                )
                ...
                if "user_answers" in step_data:
                    logging.info(
                        f"DEBUG: Updating user_answers for step {step.step_index}. "
                        f"New value: {step_data['user_answers']}"
                    )
                ...
                step.popup_chat_history = step_data.get(
                    "popup_chat_history", step.popup_chat_history
                )
                ...
                step.podcast_audio_path = step_data.get(
                    "podcast_audio_path", step.podcast_audio_path
                )
            ...
            # Note: Topic ID might not be available if topic is new and flush not
            # called?
            # We need to ensure topic is flushed.
            db.session.flush()
            content_ref = f"topic_{topic.id}_step_{step.step_index}"
            # Delete existing feedback for this step/user to overwrite with
            # current state
            Feedback.query.filter_by(
                user_id=current_user.userid, content_reference=content_ref
            ).delete()
        ...
        for card_data in incoming_cards:
            ...
            # 2. Fallback: match by Term if ID mismatch or missing (e.g. generated
            # but not saved yet)
            ...
            if matched_card:
                # Update existing
                matched_card.definition = card_data.get(
                    "definition", matched_card.definition
                )
                # Ensure we don't accidentally reset time_spent if not provided in
                # update (though it should be)
                # But if provided as 0, we might want to allow it? Usually time
                # accumulates.
                ...
                # Note: valid new_card.id won't exist until flush, but it's fine for
                # this loop

        # Delete removed flashcards
        # If it wasn't processed (updated), it means it's not in the new list, so
        # delete it.
        ...
        db.session.rollback()
        logger.error(f"Database integrity error saving topic {topic_name}: {e}")
        raise DatabaseIntegrityError(
            f"Topic '{topic_name}' may already exist or violates database constraints",
            error_code="DB100",
            debug_info={"topic_name": topic_name, "original_error": str(e)},
        )
        ...
        db.session.rollback()
        logger.error(f"Unexpected error saving topic {topic_name}: {e}", exc_info=True)
...
def save_chat_history(
    topic_name, history, history_summary=None, time_spent=0, popup_history=None
):
    """
    ...
    try:
        ...
        raise DatabaseConnectionError(
            "Unable to connect to database",
            error_code="DB103",
            debug_info={"operation": "save_chat_history", "original_error": str(e)},
        )
    except Exception as e:
        db.session.rollback()
        logger.error(f"Error saving chat history for {topic_name}: {e}", exc_info=True)
        raise DatabaseOperationError(
...
def load_topic(topic_name):
    ...
    Returns None if topic doesn't exist or user not authenticated (normal behavior
    for new topics).
    """
    ...
    data = {
        ...
        "chat_history_summary": (
            (topic.chat_mode.history_summary or []) if topic.chat_mode else []
        ),
        "popup_chat_history": (
            (topic.chat_mode.popup_chat_history or []) if topic.chat_mode else []
        ),
        "chapter_mode": [],
        "quiz_mode": None,
        "flashcard_mode": [],
    ...
    for i in range(len(plan)):
        ...
        if step_model:
            steps_data.append(
                {
                    ...
                    # Include derived fields if needed, e.g. teaching_material is
                    # actually 'content' in model?
            ...
            if step_model.podcast_audio_path:
                ...
                logging.info(
                    f"DEBUG: Processing audio for step {step_model.step_index}. "
                    f"Raw path: {audio_path}"
                )
                ...
                if not os.path.exists(audio_path):
                    ...
                    # We can also rely on flask static folder if available context,
                    # but here we are in storage.
                    candidate_path = os.path.join(
                        os.getcwd(), "app", "static", os.path.basename(audio_path)
                    )
                    logging.info(
                        f"DEBUG: Path not found. Trying candidate: {candidate_path}"
                    )
                ...
                if os.path.exists(audio_path):
                    try:
                        with open(audio_path, "rb") as audio_file:
                            ...
                            logging.info(
                                f"DEBUG: Successfully loaded and encoded audio for "
                                f"step {step_model.step_index}"
                            )
                    except Exception as e:
                        logging.warning(
                            f"Failed to load audio file for topic {topic_name} "
                            f"step {step_model.step_index}: {e}"
                        )
                else:
                    logging.warning(
                        f"Audio file not found at path: "
                        f"{step_model.podcast_audio_path} "
                        f"or resolved path {audio_path}"
                    )
    ...
    if topic.quiz_mode:
        ...
        data["quiz_mode"] = {
            ...
            "date": (
                latest_quiz.created_at.isoformat() if latest_quiz.created_at else None
            ),
...
def get_all_topics():
    ...
    try:
        ...
        topics = (
            Topic.query.with_entities(Topic.name)
            .filter_by(user_id=current_user.userid)
            .all()
        )
        ...
        raise DatabaseConnectionError(
            "Unable to connect to database",
            error_code="DB107",
            debug_info={"operation": "get_all_topics", "original_error": str(e)},
        )
    except Exception as e:
        logger.error(f"Error getting topics: {e}", exc_info=True)
        raise DatabaseOperationError(
            "Failed to retrieve topics", operation="get_all_topics", error_code="DB108"
        )
...
def delete_topic(topic_name):
    ...
    try:
        ...
        # Delete related chat sessions, chapter modes, quiz modes, flashcard modes,
        # and plan revisions
