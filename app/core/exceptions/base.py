...
class PersonalGuruException(Exception):
    ...
    def __init__(
        ...
        self.user_message = (
            user_message or "An unexpected error occurred. Please try again."
        )
    ...
    def log(self, logger: logging.Logger, endpoint: str = ""):
        ...
        session_id = session.get("_id", "no_session") if session else "no_session"

        log_message = (
            f"[{self.log_category}] [{self.error_code}] {self.__class__.__name__}: "
            f"{self.message}\n"
