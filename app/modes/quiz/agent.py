...
class QuizAgent:
    ...
    def generate_quiz(self, topic, user_background, count=10):
        ...
        if isinstance(count, str) and count.lower() == "auto":
            count = self.get_quiz_count_for_topic(topic, user_background)
        else:
        ...
        if "questions" not in quiz_data:
            print(
                f"DEBUG: 'questions' key not found. Available keys: "
                f"{list(quiz_data.keys())}"
            )
    ...
    def get_quiz_count_for_topic(self, topic, user_background=None):
        """
        Estimate the number of quiz questions needed for a topic based on its
        complexity.
