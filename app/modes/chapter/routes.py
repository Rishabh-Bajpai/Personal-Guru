...
def mode(topic_name):
    """Handle the chapter mode entry point, including plan generation or resumption."""
...
def generate_plan():
    """Generate a new study plan for the given topic."""
...
def update_plan(topic_name):
    """Update an existing study plan based on user feedback."""
...
def learn_topic(topic_name, step_index):
    """Render the learning interface for a specific plan step."""
    topic_data = load_topic(topic_name)
    if not topic_data:
        return "Topic not found", 404
    ...
    try:
        log_telemetry(
            ...
            payload={"topic": topic_name, "step_index": step_index},
        )
    except Exception:
        pass  # Telemetry failures must not block user flow; ignore logging errors.

    show_assessment = current_step_data.get("questions") and not current_step_data.get(
        "user_answers"
    )
...
def assess_step(topic_name, step_index):
    """Evaluate user answers for the assessment questions of a step."""
    topic_data = load_topic(topic_name)
    if not topic_data:
        return "Topic not found", 404
    ...
    current_step_data["popup_chat_history"] = current_step_data.get(
        "popup_chat_history", []
    )  # placeholder for template if needed
    current_step_data["feedback"] = feedback_results

    answered_questions_count = len([ua for ua in user_answers if ua])
    score = (
        (num_correct / answered_questions_count * 100)
...
def update_time(topic_name, step_index):
    ...
    if time_spent > 0:
        ...
        if topic:
            step = ChapterMode.query.filter_by(
                topic_id=topic.id, step_index=step_index
            ).first()
            if step:
...
def reset_quiz(topic_name, step_index):
    """Reset the quiz state for a specific step to allow retaking."""
    topic_data = load_topic(topic_name)
    if not topic_data:
        return "Topic not found", 404
    ...
    return redirect(
        url_for("chapter.learn_topic", topic_name=topic_name, step_index=step_index)
    )


@chapter_bp.route("/generate-audio/<int:step_index>", methods=["POST"])
def generate_audio_route(step_index):
    """Generate audio from text for a learning step."""
...
def generate_podcast_route(topic_name, step_index):
    """Generate a podcast script and audio for a topic step."""
    topic_data = load_topic(topic_name)
    if not topic_data:
    ...
    # Fallback if ID is missing (e.g. not flushed yet), though load_topic should
    ...
    if not step_id:
        ...
        return {
            "error": "Step ID not found. Please refresh the page and try again."
        }, 500
    ...
    filename = werkzeug.utils.secure_filename(
        f"podcast_{current_user.userid}_{step_id}.mp3"
    )
    ...
    # 1. Generate Script
    try:
        transcript = podcast_agent.generate_script(teaching_material, user_background)
    except Exception as error:
...
def complete_topic(topic_name):
    """Show the completion screen for a topic."""
    ...
    return render_template(
        "complete.html", topic_name=topic_name, average_score=average_score
    )
...
def export_topic_pdf(topic_name):
    ...
    if topic_data.get("plan"):
        ...
        html = render_template(
            "chapter/pdf_export.html", topic=topic_data, average_score=average_score
        )
...
def _get_topic_data_and_score(topic_name):
    """Helper function to load topic data and calculate average score."""
    topic_data = load_topic(topic_name)
    if not topic_data:
        return None, 0
