...
<html lang="en">
  <head>
    ...
    <style>
      body { font-family: sans-serif; padding: 20px; background-color: #1a1a1a;
             color: #e0e0e0; }
      ...
      .btn-danger {
          background-color: #d9534f; color: white; border: none; padding: 5px 10px;
  ...
  <body>
    ...
    <nav>
        ...
        {% for name in models %}
            <a href="/db-viewer/{{ name }}" style="{% if name == current_model %}
                font-weight: bold; color: white; border-bottom: 2px solid #00C9FF;
            {% endif %}">{{ name }}</a>
        {% endfor %}
    </nav>
    ...
    {% if current_model %}
        ...
        <div class="bulk-actions" id="bulkActions">
            <form action="/db-viewer/{{ current_model }}/bulk_delete" method="post"
            onsubmit="return confirm('Delete selected items?');">
                <input type="hidden" name="ids[]" id="bulkDeleteInput">
                <button type="submit" class="btn-danger">Delete Selected
        ...
        <div style="overflow-x: auto;">
            <table id="dataTable">
                <thead>
                    <tr>
                        <th style="width: 30px; text-align: center; cursor: default;"
                    ...
                    {% for row in rows %}
                        ...
                        onclick="toggleRow(event, this)">
                            ...
                            <td onclick="event.stopPropagation()">
                                <form action="/db-viewer/{{ current_model }}/delete/
                                {{ row['_pk_value'] }}"
        ...
        <script>
            ...
            function toggleRow(e, row) {
                // Don't trigger if clicking inside button or link or checkbox
                // container
                ...
                if (e.shiftKey && lastChecked) {
                    ...
                    // Standard file manager behavior: Click = select one
                    // (clear others), Ctrl+Click = toggle one.
            ...
            function updateBulkState() {
                ...
                // We need to append multiple inputs for form.getlist or use a
                // simpler delimiter?
    ...
    if model_name in MODELS:
        ...
        if ids:
            try:
                ...
                # model.query.filter(getattr(model, pk_name).in_(ids)).delete(
                #    synchronize_session=False)
                # ## This is faster
                # But let's stick to safe iteration for now to reuse logic or if
                # cascade needed (though delete usually cascades)

                # Fetch and delete to handle potential complex relationships if needed
                # (though batch is better)
