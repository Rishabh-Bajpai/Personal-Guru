...
# Output buffer for detailed logs (to prevent spamming stdout if not needed,
# but here we print)
...
def update_database():
    ...
    with app.app_context():
        ...
        if "study_steps" in existing_tables and "chapter_mode" not in existing_tables:
            logger.info(
                "Detected legacy table 'study_steps'. Renaming to 'chapter_mode'..."
            )
            try:
                # Rename table
                db.session.execute(
                    text("ALTER TABLE study_steps RENAME TO chapter_mode")
                )
                db.session.commit()
                logger.info(" -> Table renamed successfully.")

                # Check consistency of ID sequence if necessary
                # (usually auto-handled by serial)
        ...
        for model in TARGET_MODELS:
            ...
            if table_name == "topics":
                ...
                if not has_user_id:
                    logger.warning(
                        " !! Detected old 'topics' table schema (missing user_id). "
                        "Dropping table to recreate with proper constraints."
                    )
                    # Drop table
            ...
            # Special check for 'chat_history' and 'last_quiz_result' column removal
            # in Topics
            if table_name == "topics":
                for deprecated_col in ["chat_history", "last_quiz_result"]:
                    if deprecated_col in existing_col_map:
                        logger.info(
                            f"  [-] Dropping deprecated column: {deprecated_col}"
                        )
                        try:
                            sql = text(
                                f'ALTER TABLE "topics" DROP COLUMN "{deprecated_col}"'
                            )
                            db.session.execute(sql)
                            db.session.commit()
            ...
            # Special check for 'feedback' column removal in ChapterMode
            # (moved to table)
            if table_name == "chapter_mode":
                if "feedback" in existing_col_map:
                    logger.info(
                        "  [-] Dropping deprecated column: feedback (moved to "
                        "Feedback table)"
                    )
                    try:
            ...
            # Special check for renaming 'chat_history' -> 'popup_chat_history' in
            # ChapterMode
            if table_name == "chapter_mode":
                if (
                    "chat_history" in existing_col_map
                    and "popup_chat_history" not in existing_col_map
                ):
                    logger.info(
                        "  [~] Renaming column 'chat_history' to "
                        "'popup_chat_history'"
                    )
                    try:
                        sql = text(
                            'ALTER TABLE "chapter_mode" RENAME COLUMN "chat_history" '
                            'TO "popup_chat_history"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Renamed successfully.")
                        # Update local map
                        existing_col_map["popup_chat_history"] = existing_col_map.pop(
                            "chat_history"
                        )
                        existing_col_map["popup_chat_history"][
                            "name"
                        ] = "popup_chat_history"
                    except Exception as e:
            ...
            # Special check for 'installation_id' removal in Feedback (refactor to
            # user-centric)
            if table_name == "feedback":
                if "installation_id" in existing_col_map:
                    logger.info(
                        "  [-] Dropping deprecated column: installation_id (moved "
                        "to User-Centric schema)"
                    )
                    try:
                        sql = text(
                            'ALTER TABLE "feedback" DROP COLUMN "installation_id"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Dropped successfully.")
            ...
            if table_name == "telemetry_logs":
                ...
                if "installation_id" not in existing_col_map:
                    logger.info(
                        "  [+] Adding missing column: installation_id to telemetry_logs"
                    )
                    try:
                        # Add as nullable first
                        sql = text(
                            'ALTER TABLE "telemetry_logs" ADD COLUMN "installation_id" '
                            "VARCHAR(36)"
                        )
                        db.session.execute(sql)
                        ...
                        logger.info(
                            "      -> Backfilling installation_id from logins..."
                        )
                        sql_backfill = text(
                            """
                        ...
                        # Delete any rows that still have NULL installation_id (orphans)
                        # to allow NOT NULL constraint
                        sql_clean = text(
                            "DELETE FROM telemetry_logs WHERE installation_id IS NULL"
                        )
                        db.session.execute(sql_clean)

                        # Set NOT NULL
                        sql_const = text(
                            'ALTER TABLE "telemetry_logs" ALTER COLUMN '
                            '"installation_id" SET NOT NULL'
                        )
                        db.session.execute(sql_const)

                        # Add FK Constraint
                        sql_fk = text(
                            'ALTER TABLE "telemetry_logs" ADD CONSTRAINT '
                            "fk_telemetry_installation FOREIGN KEY (installation_id) "
                ...
                if "user_id" in existing_col_map:
                    # We can't easily check if it's nullable via Inspector in this
                    # script style without detailed reflection, but we can try to
                    # ALTER it to DROP NOT NULL blindly or check logic.
                    # For simplicity, we just run the ALTER. Postgres allows this
                    # even if already nullable.
                    try:
                        logger.info("  [*] Altering user_id to be NULLABLE")
                        sql = text(
                            'ALTER TABLE "telemetry_logs" ALTER COLUMN "user_id" '
                            "DROP NOT NULL"
                        )
                        db.session.execute(sql)
                        db.session.commit()
            ...
            # Special check for deprecated 'name' and 'password_hash' columns in User
            # table
            if table_name == "users":
                for deprecated_col in ["name", "password_hash"]:
                    if deprecated_col in existing_col_map:
                        logger.info(
                            f"  [-] Dropping deprecated column from users: "
                            f"{deprecated_col}"
                        )
                        try:
                            sql = text(
                                f'ALTER TABLE "users" DROP COLUMN "{deprecated_col}"'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                            logger.info("      -> Dropped successfully.")
            ...
            if table_name == "users":
                if (
                    "primary_language" in existing_col_map
                    and "languages" not in existing_col_map
                ):
                    logger.info(
                        "  [~] Renaming column 'primary_language' to 'languages'"
                    )
                    try:
                        sql = text(
                            'ALTER TABLE "users" RENAME COLUMN "primary_language" TO '
                            '"languages"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        ...
                        existing_col_map["languages"] = existing_col_map.pop(
                            "primary_language"
                        )
            ...
            if table_name == "logins":
                if (
                    "password" in existing_col_map
                    and "password_hash" not in existing_col_map
                ):
                    logger.info("  [~] Renaming column 'password' to 'password_hash'")
                    try:
                        sql = text(
                            'ALTER TABLE "logins" RENAME COLUMN "password" TO '
                            '"password_hash"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Renamed successfully.")
                        # Update local map
                        existing_col_map["password_hash"] = existing_col_map.pop(
                            "password"
                        )
            ...
            if table_name == "users":
                ...
                if has_username and not has_id:
                    logger.warning(
                        " !! Detected old 'users' table schema (username PK). "
                    ...
                    try:
                        ...
                        old_users_result = db.session.execute(
                            text('SELECT * FROM "users"')
                        )
                        try:
                            # SQLAlchemy 1.4+
                            users_data = [
                                dict(row._mapping) for row in old_users_result
                            ]
                        except AttributeError:
                            # Fallback
                            columns = old_users_result.keys()
                            users_data = [
                                dict(zip(columns, row)) for row in old_users_result
                            ]

                        logger.info(
                            f"    -> Found {len(users_data)} user(s) to migrate."
                        )
                        ...
                        if users_data:
                            logger.info(
                                "    -> Migrating data to new Login/User tables..."
                            )
                            migrated_count = 0
                            for u in users_data:
                                try:
                                    ...
                                    if old_username:
                                        update_topics_sql = text(
                                            "UPDATE topics SET user_id = :new_uid "
                                    ...
                                    logger.error(
                                        f"    -> Error migrating user "
                                        f"{u.get('username')}: {migration_err}"
                                    )

                            db.session.commit()
                            logger.info(
                                f"    -> Migration complete. {migrated_count} "
                                f"users migrated."
                            )
                            ...
                            try:
                                ...
                                fk_sql = text(
                                    ...
                                    "fk_topics_logins FOREIGN KEY (user_id) REFERENCES "
                                    "logins(userid)"
                                )
                                db.session.execute(fk_sql)
                                db.session.commit()
                                logger.info(
                                    "    -> Restored FK constraint on topics table."
                                )
                            except Exception as fk_err:
                                logger.warning(
                                    f"    -> Could not add FK constraint to topics "
                                    f"(non-critical): {fk_err}"
                                )
                                db.session.rollback()
            ...
            for column in model_columns:
                ...
                if col_name not in existing_col_map:
                    ...
                    # Note: This is basic. Default values and nullability might need
                    # more complex handling.
                    # For now, we add the column. If not nullable without default,
                    # Postgres will complain if table not empty.
                    ...
                    # Column exists, check for specific type updates requested (JSONB ->
                    # JSON)
                    ...
                    if "JSON" in model_type_str and "JSONB" in existing_type_str:
                        logger.info(
                            f"  [~] Converting column {col_name} from JSONB to JSON"
                        )
                        try:
                            # Cast using ::json
                            sql = text(
                    ...
                    if (
                        col_name == "languages"
                        and "VARCHAR" in existing_type_str
                        and "JSON" in model_type_str
                    ):
                        logger.info(
                            f"  [~] Converting column {col_name} from String to JSON"
                        )
                        try:
                            # Convert string "English" to JSON list ["English"]
                            # Postgres specific: using json_build_array or manual
                            # formatting
                            # Fallback logic: if empty, '[]', else '["' + val + '"]'
                            # Note: SQL injection risk minimal here as we use columns
                            # but content might need escaping if raw string concat.
                            # content might need escaping if raw string concat.
                            # Safer: USING json_build_array(languages)
                            sql = text(
                                f'ALTER TABLE "{table_name}" ALTER COLUMN "{col_name}" '
                                f'TYPE JSON USING json_build_array("{col_name}")'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                            logger.info("      -> Converted successfully.")
                    ...
                    # Special check for VARCHAR(36) -> VARCHAR(100) expansion (for
                    # userid)
                    if (
                        "VARCHAR(36)" in existing_type_str
                        and "VARCHAR(100)" in model_type_str
                    ):
                        logger.info(
                            f"  [~] Expanding column {col_name} from VARCHAR(36) to "
                            f"VARCHAR(100)"
                        )
                        try:
                            sql = text(
