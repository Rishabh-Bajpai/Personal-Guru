...
def update_database():
    ...
    with app.app_context():
        ...
        if db.engine.name == "sqlite":
            ...
            if db_uri.startswith("sqlite:///"):
                ...
                if not os.path.isabs(db_path):
                    ...
                    )  # Try instance path layout or current dir?
        ...
        if db.engine.name != "sqlite":
            if (
                "study_steps" in existing_tables
                and "chapter_mode" not in existing_tables
            ):
                logger.info(
                ...
                try:
                    ...
                    logger.exception(" -> Failed to rename table")
                    db.session.rollback()
        ...
        if db.engine.name == "sqlite":
            ...
            try:
                ...
                if "notes_shared" not in books_col_names:
                    logger.info(
                        "  [+] Adding missing column: notes_shared to books table"
                    )
                    sql = text(
                        "ALTER TABLE books ADD COLUMN notes_shared BOOLEAN DEFAULT 0 NOT NULL"
                    )
            ...
            logger.info(
                "SQLite mode: Basic updates complete. Skipping advanced schema inspections (Postgres-specific)."
            )
            return
        ...
        for model in TARGET_MODELS:
                ...
                for deprecated_col in ["chat_history", "last_quiz_result"]:
                    if deprecated_col in existing_col_map:
                        ...
                        try:
                            ...
                            logger.exception("      -> FAILED to drop column")
                            db.session.rollback()
            ...
            if table_name == "chapter_mode":
                if "feedback" in existing_col_map:
                    logger.info(
                        "  [-] Dropping deprecated column: feedback (moved to Feedback table)"
                    )
                    try:
            ...
            if table_name == "chapter_mode":
                if (
                    ...
                    logger.info(
                        "  [~] Renaming column 'chat_history' to 'popup_chat_history'"
                    )
                    try:
                        sql = text(
                            'ALTER TABLE "chapter_mode" RENAME COLUMN "chat_history" TO "popup_chat_history"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        ...
                        existing_col_map["popup_chat_history"]["name"] = (
                            "popup_chat_history"
                        )
                    except Exception:
            ...
            if table_name == "feedback":
                if "installation_id" in existing_col_map:
                    logger.info(
                        "  [-] Dropping deprecated column: installation_id (moved to User-Centric schema)"
                    )
                    try:
                        sql = text(
            ...
            if table_name == "telemetry_logs":
                ...
                if "installation_id" not in existing_col_map:
                    ...
                    try:
                        ...
                        sql = text(
                            'ALTER TABLE "telemetry_logs" ADD COLUMN "installation_id" VARCHAR(36)'
                        )
                        db.session.execute(sql)
                        ...
                        # Set NOT NULL
                        sql_const = text(
                            'ALTER TABLE "telemetry_logs" ALTER COLUMN "installation_id" SET NOT NULL'
                        )
                        db.session.execute(sql_const)

                        # Add FK Constraint
                        sql_fk = text(
                            'ALTER TABLE "telemetry_logs" ADD CONSTRAINT fk_telemetry_installation FOREIGN KEY (installation_id) REFERENCES installations(installation_id)'
                        )
                ...
                if "user_id" in existing_col_map:
                    ...
                    if not user_id_col.get(
                        "nullable", True
                    ):  # If NOT NULL (False), make it nullable
                        try:
                            ...
                            sql = text(
                                'ALTER TABLE "telemetry_logs" ALTER COLUMN "user_id" DROP NOT NULL'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                        except Exception as e:
                            logger.warning(
                                f"      -> Could not alter user_id (Lock timeout or error): {e}"
                            )
                            db.session.rollback()
                    else:
            ...
            if table_name == "users":
                if (
                    ...
                    try:
                        sql = text(
                            'ALTER TABLE "users" RENAME COLUMN "primary_language" TO "languages"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Renamed successfully.")
            ...
            if table_name == "logins":
                if (
                    ...
                    try:
                        sql = text(
                            'ALTER TABLE "logins" RENAME COLUMN "password" TO "password_hash"'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Renamed successfully.")
                    ...
                    try:
                        ...
                        if users_data:
                                ...
                                try:
                                    ...
                                    if old_username:
                                        update_topics_sql = text(
                                            "UPDATE topics SET user_id = :new_uid WHERE user_id = :old_uid"
                                        )
                                        db.session.execute(
                                            update_topics_sql,
                                            {
                                    ...
                                    logger.error(
                                        f"    -> Error migrating user {u.get('username')}: {migration_err}"
                                    )

                            db.session.commit()
                            logger.info(
                                f"    -> Migration complete. {migrated_count} users migrated."
                            )
                            ...
                            try:
                                ...
                                fk_sql = text(
                                    "ALTER TABLE topics ADD CONSTRAINT fk_topics_logins FOREIGN KEY (user_id) REFERENCES logins(userid)"
                                )
                                ...
                                logger.warning(
                                    f"    -> Could not add FK constraint to topics (non-critical): {fk_err}"
                                )
                                db.session.rollback()

                    except Exception:
                        logger.exception("    -> FATAL: Failed to migrate user data")
                        db.session.rollback()
                        db.create_all()
            ...
            for column in model_columns:
                ...
                if col_name not in existing_col_map:
                    ...
                    try:
                        ...
                        sql = text(
                            f'ALTER TABLE "{table_name}" ADD COLUMN "{col_name}" {type_str}'
                        )
                        db.session.execute(sql)
                        db.session.commit()
                        logger.info("      -> Added successfully.")
                    ...
                    if "JSON" in model_type_str and "JSONB" in existing_type_str:
                        ...
                        try:
                            # Cast using ::json
                            sql = text(
                                f'ALTER TABLE "{table_name}" ALTER COLUMN "{col_name}" TYPE JSON USING "{col_name}"::json'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                            logger.info("      -> Converted successfully.")
                        except Exception:
                            logger.exception("      -> FAILED to convert column")
                            db.session.rollback()
                    ...
                    if (
                        ...
                        try:
                            ...
                            sql = text(
                                f'ALTER TABLE "{table_name}" ALTER COLUMN "{col_name}" TYPE JSON USING json_build_array("{col_name}")'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                            logger.info("      -> Converted successfully.")
                    ...
                    if (
                        ...
                        logger.info(
                            f"  [~] Expanding column {col_name} from VARCHAR(36) to VARCHAR(100)"
                        )
                        try:
                            sql = text(
                                f'ALTER TABLE "{table_name}" ALTER COLUMN "{col_name}" TYPE VARCHAR(100)'
                            )
                            db.session.execute(sql)
                            db.session.commit()
                            logger.info("      -> Expanded successfully.")
                        except Exception:
                            logger.exception("      -> FAILED to expand column")
                            db.session.rollback()
