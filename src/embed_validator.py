...
from src.embed_test import test_embed_direct
...
def check_embed_headers(video_id: str) -> bool:
    ...
    try:
        ...
        response = requests.head(embed_url, timeout=REQUEST_TIMEOUT, allow_redirects=True)
        ...
        if x_frame_options in ['DENY', 'SAMEORIGIN']:
            logger.info(f"Video {video_id}: blocked by X-Frame-Options={x_frame_options}")
            return False
        
        # Check Content-Security-Policy for frame-ancestors directive
        if 'frame-ancestors' in csp.lower():
            # If frame-ancestors is restricted (not 'self' or '*'), likely not embeddable
        ...
        response_full = requests.get(embed_url, timeout=REQUEST_TIMEOUT, allow_redirects=True)
        if response_full.status_code >= 400:
            logger.info(f"Video {video_id}: HTTP {response_full.status_code} on embed URL")
            return False
        ...
        logger.warning(f"Video {video_id}: embed check error - {ex} (assuming embeddable)")
...
def validate_videos_batch(videos: List[Dict], session_logger=None) -> List[Dict]:
    ...
    logger.info(f"Starting validation of {len(videos)} videos (hybrid: header check + screenshot)")
    
    # Use ThreadPoolExecutor for parallel validation
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all validation tasks
        future_to_video = {executor.submit(_validate_single_video, video): video for video in videos}
        ...
        for future in as_completed(future_to_video):
            ...
            try:
                ...
                if is_playable:
                    ...
                    removed_count += 1
                    logger.warning(f"Removed video {video['id']} ({video.get('title', 'unknown')}): {reason}")
    ...
    if removed_count > 0:
        logger.info(f"Validation complete: removed {removed_count} videos, kept {len(validated)} playable")
    else:
...
def _validate_single_video(video: Dict) -> tuple:
    ...
    # Step 3: If we reach here, we trust the previous checks (or lack of failure)
