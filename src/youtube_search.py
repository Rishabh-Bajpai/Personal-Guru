...
def search_youtube_reels(topic, max_results=12):
    ...
    try:
        ...
        target_fetch = max(int(max_results +2), 12)  # Fetch +2 to account for filtering
        ...
        while len(reels) < target_fetch and attempts < 10:
            ...
            if "#shorts" not in query.lower():
                query = f"{query} #shorts" # append #shorts to query for vertical videos and remove it for any video with duration < 4 minutes
                ...
                try:
                    ...
                    for st_item in status_response.get('items', []):
                        ...
                        status_map[vid] = {
                            ...
                            'embeddable': status_obj.get('embeddable', True)  # Default to True if not specified
                        }
                except Exception as ex:
                    logger.info(f"Failed to retrieve video status: {ex}")
                    status_map = {vid: {'privacyStatus': 'public', 'embeddable': True} for vid in video_ids}
            ...
            for item in items:
                ...
                thumbnail = snippet.get('thumbnails', {}).get('medium', {}).get('url') or snippet.get('thumbnails', {}).get('default', {}).get('url') or ''
        ...
        logger.info(f"Search '{topic}': fetched {len(reels)} videos (will filter via VLM to find embeddable ones), skipped {skipped_count} private videos")
        return reels
