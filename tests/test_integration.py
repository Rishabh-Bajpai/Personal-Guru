...
def requires_llm(func):
    ...
    def wrapper(*args, **kwargs):
        if not LLM_BASE_URL or not LLM_MODEL_NAME:
            pytest.fail(
                "Test failed: LLM environment variables (LLM_BASE_URL, "
                "LLM_MODEL_NAME) not set"
            )
...
def test_quiz_agent(logger):
    ...
    with patch(
        "app.modes.quiz.agent.validate_quiz_structure", return_value=(None, None)
    ):
...
def test_feedback_agent(logger):
    ...
    # retry_agent_call is designed for single-value returns usually, or we need to
    # handle it manually.
    # Since we are essentially testing the agent logic here (which includes LLM
    # call), and FeedbackAgent handles its own retries/logic internally or relies on
    # call_llm retries, let's call it directly.
...
def test_chapter_teaching_agent(logger):
    ...
    with patch("app.modes.chapter.agent.call_llm") as mock_call:
        ...
        # We don't need retry_agent_call if we are sure it returns success
        # immediately via mock. But keeping it consistent with other tests is fine,
        # or just call directly. Direct call is simpler since mapped.
        material = agent.generate_teaching_material(
            "Science", ["Introduction"], "beginner"
        )
        error = None
...
def test_flashcard_teaching_agent(logger):
    ...
    flashcards, error = retry_agent_call(
        agent.generate_teaching_material, "Vocabulary", count=3
    )
...
def test_assessor_agent(logger):
    ...
    # Patch validation to be lenient for integration tests (on CI/CD) with smaller
    # models. The AssessorAgent using 'qwen3-vl:8b-instruct' often returns
    ...
    # validate_quiz_structure is technically not called by AssessorAgent (it does
    # simple basic validation check inline). But let's keep the call simple.
    question, error = retry_agent_call(
        agent.generate_question, "A chapter about Python.", "beginner"
    )
...
def test_generate_audio(logger):
    ...
    with (
        patch("app.common.utils.OpenAI") as MockOpenAI,
...
def test_transcribe_audio(logger):
    ...
    with patch("app.common.utils.OpenAI") as MockOpenAI:
        ...
        # Create a dummy file
        with patch(
            "builtins.open", list=True
        ):  # Just to allow open() to work if needed, or better use real temp file
            # utils.transcribe_audio opens the file. We should probably use a real
            # temp file for robustness or mock the open call inside utils.
...
def test_chat_summary_integration(auth_client, app):
    ...
    with app.app_context():
        ...
        # history_summary defaults to None in DB if just added, or [] via load_topic
        # logic. But accessing model directly (topic.chat_mode.history_summary)
        # gives raw value. It should be None or empty.
    ...
    # We patch app.common.agents.call_llm because ChatAgent (parent of chat_agent)
    # uses it.
    with (
        patch("app.common.utils.call_llm") as mock_llm_utils,
...
def test_chat_context_construction(auth_client, app):
    ...
    with (
        ...
        mock_llm_agents.return_value = "ANSWER"
    ...
    with app.app_context():
        ...
        # History: W(0), U0(1), A0(2), U1(3), A1(4), U2(5), A2(6), U3(7), A3(8)
        # = 9 messages
    ...
    with (
        ...
        # Index 4 is S1 (Summary of Assistant 1). Since mock returned "SUM", it
        # should be "SUM".
        assert passed_history[4]["content"] == "SUM"
        # Index 6 is A2 (Full Assistant 2). Since mock returned "ANSWER", it should be
        # "ANSWER".
        assert passed_history[6]["content"] == "ANSWER"
...
def test_llm_error_handling(client_no_auth, app):
    ...
    # Patch flask_login user loader or ensure bypass works
    # LOGIN_DISABLED prevents login_required, but some routes access current_user
    # attributes. We must patch current_user where it is used.
    ...
    with (
        ...
        with patch(
            ...
            with patch("app.common.utils.get_user_context", return_value="Beginner"):
                ...
                assert (
                    b"AI Service is currently unavailable" in response.data
                    or b"LLM503" in response.data
                )
