...
def test_full_learning_flow(auth_client, mocker, logger):
    ...
    # chapter.mode:
    #   planner.generate_study_plan -> saves topic ->
    #   redirect(url_for('chapter.learn_topic', ...))
    ...
    # Check for the div and content separately to handle formatting/whitespace changes
    assert b'<div id="step-content-markdown" style="display: none">' in response.data
    assert b"## Step Content" in response.data
    ...
    # Check that assessment form is not shown (action URL absent implies form
    # absent or different)
...
def test_log_capture_threading():
    """Test that log capture correctly buffers and flushes logs using background
    thread."""
    ...
    with (
        ...
        capture = LogCapture(
            None
        )  # don't attach to real app to avoid interfering with other tests
        ...
        assert "Log 1" in [
            log_item["message"].strip() for log_item in log_entry.payload["logs"]
        ]
